...
export function FavoritesProvider({ children }: { children: React.ReactNode }) {
  // Start empty so static-export SSR matches first-paint render.
  // Hydrate from localStorage in the effect below.
  ...
  // Hydrate from localStorage on mount (post-hydration, so SSR sees [], []).
  useEffect(() => {
  ...
  useEffect(() => {
    const onStorage = (e: StorageEvent) => {
      if (
        e.key === "embodied-arxiv/favorites/v1" ||
        e.key === "embodied-arxiv/feedback/v1"
      ) {
        reload();
      }
    };
  ...
  const movePaperFn = useCallback(
    async (arxivId: string, folderId: string | null) => {
