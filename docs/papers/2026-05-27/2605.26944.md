...
- <span class="trick-core">⭐ 核心</span> — **将抓取任务分解为位姿/形状估计+反平行采样，模块化方法优于端到端方法**
- 对比编码器-解码器（如SAM3D）和扩散模型（如InstantMesh）两种位姿/形状估计方式
- 利用视觉语言模型增强单视图RGB-D输入，实现语言条件抓取

## 📝 摘要

=== "中文"

    物体位姿与形状估计问题近期取得了关键进展。编码器-解码器模型（如SAM3D、LRM、CRISP）和扩散模型（如InstantMesh、Zero123、SceneComplete）展示了类别无关的形状编码能力和开放集泛化能力。本文提出一个问题：物体位姿与形状估计方法是否足够成熟，以至于与反平行抓取采样结合使用时，能够超越端到端抓取合成方法？我们通过将研究范围限定在平行夹爪、7自由度抓取和单视图RGB(-D)图像输入，详细探讨了这一问题。我们实现并比较了一种最先进的端到端抓取合成方法和三种模块化方法，后者首先估计场景中所有物体的位姿和形状，然后使用反平行采样生成抓取。我们观察到，在所有实验中，模块化方法均优于端到端方法。模块化方法能够合成大量抓取，即使是端到端方法失败的小物体也能成功。模块化方法的有效性取决于位姿和形状估计的准确性，并在杂乱场景中部分退化——这是现有位姿和形状估计方法的局限性。我们还分析了三种模块化方法的失败模式和运行时间，它们使用了两种不同的物体位姿和形状估计方式：一种基于编码器-解码器模型，另一种基于扩散模型。最后，我们证明，单视图物体位姿和形状估计方法可以与视觉语言模型结合，仅从单视图RGB-D图像输入即可生成语言条件抓取。我们注意到其性能与最先进的LERF-TOGO基线相当。
...
与端到端抓取合成方法（如GraspNet）相比，模块化方法通过显式位姿/形状估计实现更好的泛化。与LERF-TOGO等语言条件抓取方法相比，本文方法利用现成视觉语言模型实现类似功能。

<div class="critique-box">
<div class="critique-header"><span class="critique-emoji">👀</span> 锐评</div>
<div class="critique-body">实验设计扎实，对比了模块化与端到端方法，并分析了失败模式。但位姿/形状估计方法的选择（SAM3D、InstantMesh等）并非最新，且缺乏对更复杂场景（如动态物体）的评估。此外，语言条件抓取部分仅与LERF-TOGO比较，未与更多基线对比。</div>
</div>

---

**Tags**: `grasping` `pose-estimation` `shape-estimation` `modular` `vision-language`
